...
df[(df['test preparation course'] == 'completed')].groupby(by = 'parental level of education')['AvgResults'].mean().plot(
    kind = 'barh', 
    figsize = (8, 5), 
    grid = True,
    title="AvgResults (completed course) by parental level of education",
    xlabel="AvgResults",
    ylabel="parental level of education")
plt.show()
...
# будуємо ступінчасту діаграму для візуалізації результатів
df[(df['test preparation course'] == 'none')].groupby(by = 'parental level of education')['AvgResults'].mean().plot(
    kind = 'barh', 
    figsize = (8, 5), 
    grid = True,
    title="AvgResults (not completed) by parental level of education",
    xlabel="AvgResults",
    ylabel="parental level of education")
plt.show()
...
df.groupby(by = 'lunch')['AvgResults'].mean().plot(
    kind = 'pie', 
    title="Average results depending on breakfast eaten before the exam")
plt.show()
...
df[df['lunch'] == 'free/reduced']['AvgResults'].plot(kind = 'box', title="Average results of applicants who had a free/reduced lunch")
plt.show()
# будуємо коробку з вусами 2 для візуалізації результатів
df[df['lunch'] == 'standard']['AvgResults'].plot(kind = 'box', title="Average results of applicants who had a standard lunch")
plt.show()
